    ...
    for line in lines:
        ...
        if not parsed_line_url.netloc:
            if is_proxy:
                ...
                modified_line = f"{parsed_url.scheme}://{domain_port}{base_path}/{line}"
        else:
            if is_proxy:
                modified_line = f"{PROXY_URL}/data/{parsed_line_url.hostname}:{parsed_line_url.port or (443 if parsed_line_url.scheme == 'https' else 80)}{parsed_line_url.path}"
            else:
                modified_line = line
